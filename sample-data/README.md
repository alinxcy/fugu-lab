...
- `usage.sample.jsonl` — Phase 0 の動作確認・ミニベンチ・分散/外注実験で出た実レコード **39 件**。

## この39件の内訳(ダッシュボード開発の良いテストケース)

| 種別 | 件数 | 特徴 |
|---|---|---|
| fugu-ultra | 16 | 裏率 78〜97%。裏 1,260〜52,001。elapsed 5.6〜408.7s。本文空の行あり |
| fugu | 21 | ほぼ裏 0%。elapsed 2〜29s(抽出/コード/翻訳/要約/正規表現など) |
